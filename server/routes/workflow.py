...
def refine_prompt(req: RefinePromptRequest):
    ...
    # Default to full gpt-4o for rewrite quality — mini tends to drop the
    # 邊界與限制 section and produce flat prose instead of structured output.
    # Override via OPENAI_MODEL_PROMPT_REFINE if you want cheaper/faster.
    model = _os.getenv("OPENAI_MODEL_PROMPT_REFINE") or _os.getenv("OPENAI_MODEL") or "gpt-4o"
    ...
    system_text = (
        ...
        "# Goal\n"
        "優化用戶輸入的任務描述，輸出必須同時涵蓋以下 4 個要素（缺一不可）：\n"
        "1. **明確的操作對象**：指出具體的數據來源或目標平台。若用戶沒講，**主動補上合理預設**"
        "（例：提到「經濟新聞」→ 建議 `cnyes.com, ltn.com.tw, money.udn.com`；"
        "提到「會議記錄」→ 建議 Notion DB）。絕對不要使用「指定的 XXX」這種佔位詞。\n"
        "2. **具體的數量與規格**：明確數字、長度、格式（如：10 則、每則 200 字元以上、PDF、A4 直式）。\n"
        "3. **邏輯步驟**：拆解為「獲取 → 處理 → 輸出」的清晰流程，每步用動作動詞開頭"
        "（抓取 / 彙整 / 轉換 / 寫入 / 推送 / 通知）。\n"
        "4. **邊界與限制**：設定異常處理、去重規則、時效過濾。用戶若沒講，**必須**自行補上合理規則"
        "（例：新聞 24h 內、去除重複來源、網頁無法解析則跳過並記錄）。\n\n"
        "# Guidelines\n"
        "- **保持簡練但精確**：不冗長、不空話，每句都要有資訊量。\n"
        "- **結構化輸出**：使用條列格式，不要寫成一整段散文。\n"
        "- **保留用戶核心意圖**：不得替用戶改變目標、加入用戶沒要求的新功能。\n"
        "- **能排程就標時間**：用戶若提到「每天」「每週 X」「早上 X 點」，在輸出中明確寫出排程條件。\n\n"
        "# Output Format（嚴格遵守）\n"
        "用下列固定結構輸出純文字，不要 code fence、不要開場白、不要結尾總結：\n\n"
        "【任務】一句話概括最終目標（含時間條件，若有）\n"
        "【來源】具體數據來源 / 目標平台（網站 domain、服務名稱）\n"
        "【規格】數量、長度、格式要求（條列）\n"
        "【步驟】\n"
        "  1. 動詞開頭 — 第一步要做什麼\n"
        "  2. 動詞開頭 — 第二步\n"
        "  3. 動詞開頭 — 第三步（可視複雜度增減）\n"
        "【限制】異常處理、去重、時效等邊界規則（條列）\n\n"
        "# 範例\n"
        "用戶輸入：\n"
        "  「每個工作天上午八點推送十則最新的經濟新聞給我，並總結重點，每則新聞的重點不可低於200字元，並整理成PDF檔案給我下載」\n\n"
        "優化後輸出：\n"
        "【任務】每個工作天 08:00 自動推送當日台灣經濟新聞摘要 PDF\n"
        "【來源】cnyes.com、money.udn.com、ltn.com.tw、wealth.com.tw\n"
        "【規格】\n"
        "  - 新聞數量：10 則\n"
        "  - 每則重點長度：≥ 200 字元（以中文字數計）\n"
        "  - 輸出格式：PDF（A4 直式，含標題、來源、發布時間、重點摘要）\n"
        "【步驟】\n"
        "  1. 抓取 — 從上述 4 個來源搜尋當日「台灣 經濟 最新」共 10 則（去重後取前 10）\n"
        "  2. 彙整 — 對每則新聞產出 ≥ 200 字元的重點摘要（保留關鍵數字與人物）\n"
        "  3. 轉換 — 依格式生成 PDF，檔名格式 `經濟新聞_YYYYMMDD.pdf`\n"
        "  4. 推送 — 將 PDF 下載連結回傳給使用者\n"
        "【限制】\n"
        "  - 僅取發布時間在 24 小時內的新聞\n"
        "  - 重複標題 / 同來源同主題視為同一則\n"
        "  - 若某則新聞網頁無法解析則跳過並在 log 記錄原 URL\n"
        "  - 若當日可抓新聞 < 10 則，取實際可抓到的全部即可，不強求湊滿"
    )
    ...
    try:
        resp = client.chat.completions.create(
            ...
            temperature=0.3,      # low — stick to the structure
            max_tokens=1200,      # wider room so 邊界與限制 doesn't get cut
        )
    except Exception as e:
