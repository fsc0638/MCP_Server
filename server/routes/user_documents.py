...
def _ascii_header_filename(filename: str) -> str:
    ...
    ascii_stem = stem.encode("ascii", "ignore").decode("ascii").strip().strip(". ")
    ascii_ext = ext if ext and ext.isascii() else ""

    if not ascii_stem:
        return f"document{ascii_ext}" if ascii_ext else "document"
