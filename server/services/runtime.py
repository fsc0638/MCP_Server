...
def get_universal_system_prompt(platform: str = "web", language: str = "繁體中文", detail_level: str = "適中") -> str:
    ...
    if platform == "line":
        line_tone_instruction = (
            ...
            "- 不熟悉系統的用戶 → 多一點引導，語氣溫和\n"
            "\n【短回覆上下文串接 — 極重要】\n"
            "當使用者回覆非常簡短的訊息（如數字「10」、「5分鐘」、「好的」、「第2個」），\n"
            "你必須結合前幾輪對話理解使用者的完整意圖，而非將短回覆視為獨立的新指令。\n"
            "例如：\n"
            "- 你問「請問幾分鐘後推送？」→ 使用者回「10」→ 意思是「10分鐘後」，你應建立 cron='once +10m' 的排程\n"
            "- 你問「要刪除哪一個？」→ 使用者回「1」→ 意思是「刪除第1項」\n"
            "- 你問「需要設定排程嗎？」→ 使用者回「好的」→ 意思是「確認執行上文提到的操作」\n"
            "⚠️ 嚴禁將短回覆當成全新指令。必須從對話歷史中提取完整需求再執行。\n"
            "\n【排程刪除/暫停/恢復 — 必須先確認】\n"
