...
class ScheduledPushService:
    ...
    def _build_task_prompt(self, task_type: str, config: dict, today: str) -> str:
        ...
        if task_type == "news":
            ...
            if needs_pdf:
                prompt += (
                    f"【步驟四：製作 PDF — 一次完成，禁止分批】\n"
                    f"使用 mcp-python-executor 將**全部 {count} 則**新聞製作成**一個** PDF 檔案。\n"
                    f"⚠️ 關鍵規則：\n"
                    f"- 必須在**一次** mcp-python-executor 呼叫中寫入全部 {count} 則新聞\n"
                    f"- 禁止分批（先5則再5則）— 必須一次性將所有新聞寫入同一個 PDF\n"
                    f"- PDF 內容深度必須與摘要完全一致（{level['label']}模式 = {level['sentences']}）\n"
                    f"- 不可以在 PDF 中再次精簡，每則新聞必須包含：\n"
                    f"  1. 完整標題\n"
                    f"  2. 完整摘要內容（與步驟二相同的完整句數，逐字寫入 PDF）\n"
                    f"  3. 來源名稱與個別文章 URL\n"
                    f"- 存放到 downloads 目錄，並在最終回覆中附上下載連結\n"
                    f"- 如果內容太長無法一次寫入 Python 字串，用列表逐則 append 再迴圈寫入 PDF\n\n"
                    f"PDF 程式碼範例結構（僅供參考，請用實際新聞內容填入）：\n"
                    f"```python\n"
                    f"news_list = [\n"
                    f"    {{'title': '新聞標題1', 'body': '完整摘要...', 'source': '來源', 'url': 'https://...'}},\n"
                    f"    # ... 共 {count} 則\n"
                    f"]\n"
                    f"for i, news in enumerate(news_list, 1):\n"
                    f"    pdf.chapter_title(f'{{i}}. {{news[\"title\"]}}')\n"
                    f"    pdf.chapter_body(news['body'])\n"
                    f"    pdf.chapter_body(f'🔗 {{news[\"source\"]}}\\n{{news[\"url\"]}}')\n"
                    f"    pdf.add_separator()\n"
                    f"```\n\n"
                )
            prompt += (
