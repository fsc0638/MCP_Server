    ...
    def _build_task_prompt(self, task_type: str, config: dict, today: str) -> str:
        ...
        if task_type == "news":
            ...
            prompt = (
                ...
                f"🌏 地區強制規則：\n"
                f"  - 每次呼叫 mcp-web-search 時，query 必須加上「台灣」前綴（例如：query=\"台灣 {topic} 最新\"）\n"
                f"  - 同時傳入 include_domains 參數限制來源：include_domains=[\"udn.com\", \"ltn.com.tw\", \"ettoday.net\", \"chinatimes.com\", \"cna.com.tw\", \"tvbs.com.tw\", \"setn.com\", \"storm.mg\", \"news.pts.org.tw\", \"nhk.or.jp\", \"japantimes.co.jp\"]\n"
                f"  - 若使用者有指定特定區域、國家或新聞入口網，以使用者指定為主，否則一律使用上述台灣/日本來源\n\n"
                f"使用 mcp-web-search 搜尋今日「{topic}」最新新聞，至少搜尋 {level['search_rounds']} 次（不同關鍵字），直到蒐集滿 {count} 則不重複新聞為止（最多搜尋 6 次）。\n"
                f"搜尋關鍵字必須包含「台灣」+「{topic}」相關詞彙，禁止用無關主題（如財經、科技）搜尋。\n"
